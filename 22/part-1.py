...
class Solver:
    ...
    def run(self):
        ...
        for brick in self.data:
            print(brick, 'above: ', brick.bricks_above)
            safe = True
            for above_brick in brick.bricks_above:
                assert above_brick.min_z == brick.max_z + 1
                for below_brick in above_brick.bricks_below:
                    assert below_brick.max_z + 1 == above_brick.min_z
                if len(set([b.id for b in above_brick.bricks_below])) == 1:
                    safe = False
                    break
            if safe is True:
                print('safe')
